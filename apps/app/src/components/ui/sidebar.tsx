...
type SidebarContextProps = {
  ...
  openLeftMobile: boolean;
  openRightMobile: boolean;
  setOpenLeftMobile: (open: boolean) => void;
  setOpenRightMobile: (open: boolean) => void;
  isMobile: boolean;
...
function SidebarProvider({
  ...
  const [openLeftMobile, setOpenLeftMobile] = React.useState(false);
  const [openRightMobile, setOpenRightMobile] = React.useState(false);
    ...
    <SidebarContext.Provider value={contextValue}>
      <TooltipProvider delayDuration={0}>
        <div
          ...
          className={cn(
            "group/sidebar-wrapper has-data-[variant=inset]:bg-sidebar relative flex h-svh w-full overflow-hidden",
            className,
          )}
          {...props}
        >
...
function Sidebar({
  ...
  const openMobile = side === "left" ? openLeftMobile : openRightMobile;
  const setOpenMobile =
    side === "left" ? setOpenLeftMobile : setOpenRightMobile;
